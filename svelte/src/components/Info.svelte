<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
    <!-- Card 1 -->
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
      <div class="p-4">
        <div class="flex items-center justify-center my-3">
            <svg class="w-8 h-8 mr-3" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" version="1.1" viewBox="0 0 373.833 373.833" xml:space="preserve">
                <g>
                    <path d="M373.833,173.995h-0.015v-91.34l-3.662-2.535c-2.478-1.715-62.077-42.009-183.236-42.009    C65.749,38.11,6.15,78.404,3.672,80.119L0.01,82.655v99.841c0,0.002,0,0.004,0,0.006L0,318.341h0.916v8.882    c0,4.694,3.806,8.5,8.5,8.5s8.5-3.806,8.5-8.5v-8.882h338v8.882c0,4.694,3.806,8.5,8.5,8.5s8.5-3.806,8.5-8.5v-8.882h0.899    L373.833,173.995z M309.833,173.995h-115.42v-38.221c12.583-1.84,39.075-5.373,57.002-5.373c17.916,0,45.47,3.577,58.418,5.41    V173.995z M176.836,173.995H61.416v-38.221c12.583-1.84,39.075-5.373,57.002-5.373c17.916,0,45.47,3.577,58.418,5.41V173.995z     M356.831,190.995l-0.003,23.345H17.008l0.002-23.345H356.831z M17.007,231.341h339.819l-0.008,70H17.001L17.007,231.341z     M186.909,55.11c99.044,0,155.556,28.461,169.909,36.75v82.135h-29.984v-52.798l-7.187-1.124    c-1.741-0.272-42.935-6.671-68.231-6.671c-23.526,0-59.163,5.459-65.81,6.51c-6.813-1.039-43.796-6.51-67.187-6.51    c-25.478,0-65.188,6.406-66.866,6.679l-7.136,1.16v52.754H17.01V91.862C31.388,83.558,87.896,55.11,186.909,55.11z"/>
                    <line x1="186.909" y1="38.11" x2="186.919" y2="38.11"/>
                </g>
            </svg>
            <p class="text-xl">1</p>
        </div>
        <h3 class="text-xl font-medium text-gray-800">King's sized bed</h3>
      </div>
    </div>
  
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="p-4">
          <div class="flex items-center justify-center my-3">
            <svg class="w-8 h-8 mr-3" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" version="1.1" id="Capa_1" viewBox="0 0 228.538 228.538" xml:space="preserve">
                <g>
                    <path d="M228.538,116.279c0-10.345-8.417-18.762-18.763-18.762H31.487V36.535c0-7.02,5.711-12.731,12.731-12.731h3.333    c3.449,0,6.703,1.402,9.066,3.795l-0.565,0.565c-9.489,9.489-9.489,24.928,0,34.418l3.219,3.219    c1.419,1.419,3.279,2.129,5.14,2.129c1.861,0,3.72-0.709,5.14-2.129l31.782-31.782c2.838-2.838,2.838-7.441,0-10.279l-3.219-3.219    c-4.596-4.597-10.708-7.128-17.208-7.128c-4.969,0-9.708,1.483-13.719,4.233c-5.077-5.264-12.138-8.359-19.635-8.359h-3.333    c-15.036,0-27.269,12.232-27.269,27.269V97.61C7.452,98.525,0,106.548,0,116.28c0,7.959,4.99,14.76,12,17.481v23.956    c0,21.129,15.039,38.812,34.975,42.91l-6.236,6.236c-2.838,2.838-2.838,7.44,0,10.278c1.419,1.419,3.279,2.129,5.14,2.129    c1.861,0,3.72-0.709,5.14-2.129l15.628-15.62h95.24l15.633,15.62c1.418,1.419,3.278,2.129,5.14,2.129    c1.86,0,3.72-0.709,5.14-2.129c2.838-2.838,2.838-7.44,0-10.278l-6.236-6.236c19.938-4.098,34.976-21.78,34.976-42.91v-23.956    C223.547,131.038,228.538,124.239,228.538,116.279z M80.906,27.929c1.686,0,3.306,0.422,4.74,1.217L64.683,50.109    c-2.055-3.725-1.505-8.513,1.65-11.667l7.644-7.644C75.828,28.948,78.289,27.929,80.906,27.929z M18.763,112.055h191.012    c2.33,0,4.226,1.895,4.226,4.226c0,2.329-1.896,4.225-4.226,4.225H18.763c-2.33,0-4.225-1.896-4.225-4.225    C14.538,113.95,16.433,112.055,18.763,112.055z M202,157.716c0,16.138-13.131,29.268-29.271,29.268H55.808    c-16.14,0-29.271-13.129-29.271-29.268v-22.465H202V157.716z"/>
                    <path d="M92.059,82.698c0.688,0.688,1.589,1.032,2.491,1.032c0.902,0,1.803-0.344,2.491-1.032c1.376-1.376,1.376-3.606,0-4.982    l-3.55-3.55c-1.376-1.376-3.606-1.376-4.982,0c-1.376,1.376-1.376,3.606,0,4.982L92.059,82.698z"/>
                    <path d="M102.708,79.147l3.55,3.55c0.688,0.688,1.589,1.032,2.491,1.032s1.804-0.344,2.492-1.032c1.375-1.376,1.375-3.607,0-4.983    l-3.55-3.55c-1.376-1.376-3.607-1.375-4.983,0C101.332,75.541,101.332,77.771,102.708,79.147z"/>
                    <path d="M89.941,63.515l-3.55-3.55c-1.376-1.376-3.606-1.376-4.982,0c-1.376,1.376-1.376,3.605,0,4.982l3.55,3.55    c0.688,0.688,1.589,1.032,2.491,1.032c0.902,0,1.803-0.344,2.491-1.032C91.317,67.122,91.317,64.891,89.941,63.515z"/>
                    <path d="M100.591,59.965c-1.376-1.376-3.606-1.376-4.982,0c-1.376,1.376-1.376,3.605,0,4.982l3.55,3.55    c0.688,0.688,1.589,1.032,2.491,1.032c0.902,0,1.803-0.344,2.491-1.032c1.376-1.376,1.376-3.606,0-4.982L100.591,59.965z"/>
                    <path d="M95.395,45.552c-1.375,1.376-1.375,3.607,0,4.983l3.55,3.55c0.688,0.688,1.589,1.032,2.491,1.032s1.804-0.344,2.492-1.032    c1.375-1.376,1.375-3.607,0-4.983l-3.55-3.55C99.002,44.176,96.771,44.176,95.395,45.552z"/>
                    <path d="M112.671,64.251c0.688,0.688,1.589,1.032,2.491,1.032c0.902,0,1.804-0.344,2.492-1.032c1.375-1.376,1.375-3.607,0-4.983    l-3.55-3.55c-1.376-1.376-3.607-1.375-4.983,0c-1.375,1.376-1.375,3.607,0,4.983L112.671,64.251z"/>
                </g>
                </svg>
              <p class="text-xl">1</p>
          </div>
          <h3 class="text-xl font-medium text-gray-800">Bathroom</h3>
        </div>
    </div>

    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="p-4">
          <div class="flex items-center justify-center my-3">
            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-8 h-8" viewBox="0 0 16 16">
                <path d="M15.384 6.115a.485.485 0 0 0-.047-.736A12.444 12.444 0 0 0 8 3C5.259 3 2.723 3.882.663 5.379a.485.485 0 0 0-.048.736.518.518 0 0 0 .668.05A11.448 11.448 0 0 1 8 4c2.507 0 4.827.802 6.716 2.164.205.148.49.13.668-.049z"/>
                <path d="M13.229 8.271a.482.482 0 0 0-.063-.745A9.455 9.455 0 0 0 8 6c-1.905 0-3.68.56-5.166 1.526a.48.48 0 0 0-.063.745.525.525 0 0 0 .652.065A8.46 8.46 0 0 1 8 7a8.46 8.46 0 0 1 4.576 1.336c.206.132.48.108.653-.065zm-2.183 2.183c.226-.226.185-.605-.1-.75A6.473 6.473 0 0 0 8 9c-1.06 0-2.062.254-2.946.704-.285.145-.326.524-.1.75l.015.015c.16.16.407.19.611.09A5.478 5.478 0 0 1 8 10c.868 0 1.69.201 2.42.56.203.1.45.07.61-.091l.016-.015zM9.06 12.44c.196-.196.198-.52-.04-.66A1.99 1.99 0 0 0 8 11.5a1.99 1.99 0 0 0-1.02.28c-.238.14-.236.464-.04.66l.706.706a.5.5 0 0 0 .707 0l.707-.707z"/>
            </svg>
          </div>
          <h3 class="text-xl font-medium text-gray-800">Free wi-fi</h3>
        </div>
    </div>

    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="p-4">
          <div class="flex items-center justify-center my-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="mr-3 w-8 h-8" fill="currentColor" viewBox="0 0 16 16">
                <path d="M11 2a3 3 0 0 1 3 3v6a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3V5a3 3 0 0 1 3-3h6zM5 1a4 4 0 0 0-4 4v6a4 4 0 0 0 4 4h6a4 4 0 0 0 4-4V5a4 4 0 0 0-4-4H5z"/>
            </svg>
            <p class="text-xl">20 m&sup2;</p>
          </div>
          
          <h3 class="text-xl font-medium text-gray-800">Dimensions </h3>
        </div>
    </div>
    
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="p-4">
          <div class="flex items-center justify-center my-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" version="1.1" id="Layer_1" viewBox="0 0 330 330" xml:space="preserve">
                <g id="XMLID_221_">
                    <path id="XMLID_222_" d="M285,250h-29.19l-16.58-49.744C237.187,194.131,231.456,190,225,190h-75v-49.871h75   c8.283,0,15-6.716,15-15s-6.717-15-15-15h-75V87.42c17.458-6.192,30-22.865,30-42.42c0-24.813-20.188-45-45-45   c-24.814,0-45,20.188-45,45c0,19.555,12.541,36.228,30,42.42v24.396C68.521,121.24,30,166.277,30,220c0,60.654,49.346,110,110,110   c39.826,0,75.394-21.537,94.712-54.096C237.434,278.471,241.08,280,245,280h40c8.283,0,15-6.716,15-15S293.283,250,285,250z    M140,300c-44.113,0-80-35.888-80-80c0-37.155,25.328-68.595,60-77.483V205c0,8.284,6.715,15,15,15h79.187l4.613,13.84   C212.189,271.782,178.889,300,140,300z"/>
                </g>
            </svg>
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 16 16">
                <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z"/>
            </svg>
        </div>
          
          <h3 class="text-xl font-medium text-gray-800">Flat studio</h3>
        </div>
    </div>
  
    <!-- Add more cards for other features -->
</div>